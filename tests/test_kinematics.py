...
def test_urdf():
    ...
    th_batch = torch.rand(
        N, len(chain.get_joint_parameter_names()), dtype=dtype, device=d
    )
...
def test_fk_simple_arm():
    ...
    ret = chain.forward_kinematics(
        {"arm_elbow_pan_joint": math.pi / 2.0, "arm_wrist_lift_joint": -0.5}
    )
    tg = ret["arm_wrist_roll"]
    pos, rot = quat_pos_from_transform3d(tg)
    assert quaternion_equality(rot, torch.tensor([0.70710678, 0.0, 0.0, 0.70710678]))
...
def test_cuda():
    if torch.cuda.is_available():
        ...
        pos, rot = quat_pos_from_transform3d(tg)
        assert quaternion_equality(
            rot, torch.tensor([0.70710678, 0.0, 0.0, 0.70710678], dtype=dtype, device=d)
        )
...
def test_fk_mjcf():
    ...
    tg = ret["aux_1_child"]
    pos, rot = quat_pos_from_transform3d(tg)
    assert quaternion_equality(rot, torch.tensor([0.87758256, 0.0, 0.0, 0.47942554]))
    assert torch.allclose(pos, torch.tensor([0.2, 0.2, 0.75]))
    tg = ret["front_left_foot_child"]
    pos, rot = quat_pos_from_transform3d(tg)
    assert quaternion_equality(
        rot, torch.tensor([0.77015115, -0.4600326, 0.13497724, 0.42073549])
    )
